...
def process_row(params):
    ...
    mean_error_main = np.sqrt(np.mean(squared_errors))
    percentage_error_main = (mean_error_main / np.sqrt(1600*1200)) * 100
    ...
    temp_path = f"{root_path}data/{markers_finetune[map_finetune[subject]][f'C{main}']['path']}"
    new_points = []
    for id in temp_ids:
    ...
    mean_error_skel_main = np.sqrt(np.mean(err_skel))
    ...
    mean_error_pair = np.sqrt(np.mean(squared_errors))
    percentage_error_pair = (mean_error_pair / np.sqrt(1600*1200)) * 100
    ...
    new_points = np.array(new_points)
    err_skel = np.sum((new_points - temp_points)**2, axis=1)
    mean_error_skel_pair = np.sqrt(np.mean(err_skel))
    ...
    data_res = {
        ...
        'path': path,
        'subject': subject,
        'error': mean_error_main,
        'perc_error': percentage_error_main,
        'error_pair': mean_error_pair,
        'perc_error_pair': percentage_error_pair,
        'error_skel': mean_error_skel_main,
        'error_skel_pair': mean_error_skel_pair
    }
    pbar.update(1)
    return data_res
...
with ThreadPoolExecutor() as executor:
    # Use map to apply process_row to each row in df
    processed_rows = list(executor.map(process_row, ((idx, row) for idx, row in df.iterrows())))

# Filter out None results (in case of errors)
results = [result for result in processed_rows if result is not None]

df_temp = pd.DataFrame(results)
df = pd.merge(df, df_temp, on=['main', 'pair', 'repeat', 'num_frames', 'path', 'subject'])

df.to_csv(f'{root_path}calibration/external/csvs/external_calibrations_w_errors_w_markers.csv', index=False)

idx = df.groupby(['subject', 'main', 'pair'])['error_skel'].idxmin()
filtered_df = df.loc[idx]
filtered_df.to_csv(f'{root_path}calibration/external/csvs/external_calibrations_w_errors_w_markers_best_main.csv', index=False)

idx = df.groupby(['subject', 'main', 'pair'])['error_skel_pair'].idxmin()
filtered_df = df.loc[idx]
filtered_df.to_csv(f'{root_path}calibration/external/csvs/external_calibrations_w_errors_w_markers_best_pair.csv', index=False)